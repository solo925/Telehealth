...
def book_appointment(request):
    """
    API to book an appointment and create Twilio video room and chat service.
    """
...
def appointment_list(request):
    """
    API to get a list of appointments for the logged-in user.
    """
    appointments = Appointment.objects.filter(patient__user=request.user)
...
def appointment_detail(request, pk):
    """
    API to get the details of a specific appointment.
    """
    appointment = get_object_or_404(Appointment, pk=pk, patient__user=request.user)
...
def appointment_video_chat(request, appointment_id):
    """
    API to get access token for video chat of a specific appointment.
    """
